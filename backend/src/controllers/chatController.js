...
const updateSessionTitle = async (req, res) => {
  try {
    ...
    res.json({ message: "Judul berhasil diubah", title: newTitle });
  } catch (error) {
    console.error("Error update title:", error);
    res.status(500).json({ error: "Gagal mengubah judul" });
  }
};
...
const deleteSession = async (req, res) => {
  try {
    ...
    // Opsional: Hapus sub-collection messages secara manual jika perlu (Firestore tidak auto-cascading delete)
    // Tapi untuk mock/mvp ini cukup hapus parent doc reference.
...
// --- 5. Handle Summary (Tetap sama) ---
const handleSummary = async (req, res) => {
  try {
    const chatHistory = req.body.history || [];

    const journalsRef = db.collection("journals");
    // Ambil start date (7 hari yang lalu)
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

    // Query Firestore: Filter server-side jika memungkinkan, atau client-side filter
    // Note: Firestore filter 'where' dengan 'orderBy' butuh index. 
    // Untuk safety MVP, kita ambil semua (descending update) lalu filter manual di loop.
    ...
    snapshot.forEach((doc) => {
      ...
      // Pastikan field di database adalah 'created_at' dan tipe Timestamp
      const createdAt = data.created_at ? data.created_at.toDate() : new Date();

      if (createdAt >= sevenDaysAgo) {
        journalEntries.push(`--- JOURNAL ENTRY (${createdAt.toISOString().split('T')[0]}) ---\nTitle: ${data.title}\nContent: ${data.content}\n--- END JOURNAL ENTRY ---`);
      }
    });
